...
export default function NewDailyHypeButton({ userId }: { userId: string }) {
  ...
  const [type, setType] = useState<'career'|'school'|'sports'|'personal'>('career')
  const [eventAt, setEventAt] = useState<string>('') // ISO string
  const [visibility, setVisibility] = useState<'public'|'followers'|'following'|'private'>('public')
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast('Daily hype created!')
      setOpen(false)
      setTitle(''); setType('career'); setEventAt(''); setVisibility('public')
    } catch (err) {
      console.error(err)
      toast('Error creating daily hype')
    } finally {
      setSaving(false)
    }
  }
          ...
          <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[color:var(--brand-dark)] text-white">
            ...
            <form onSubmit={submit} className="p-4 space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => setOpen(false)} className="px-4 py-2 rounded-full border border-white/20 bg-white/5 hover:bg-white/10">
                  Cancel
                </button>
                <button type="submit" disabled={saving} className="px-4 py-2 rounded-full brand-gradient text-white shadow-brand disabled:opacity-60">
